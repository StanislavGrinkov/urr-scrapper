...
def get_args():
    ...
    parser.add_argument('--outdir', nargs='?', default='articles_gsg')
...
def nav_link_extract_prev(dom):
    return nav_link_extract(dom, 'div.nav-previous > a[href]')


def nav_link_extract_next(dom):
    return nav_link_extract(dom, 'div.nav-next > a[href]')
...
def process_article_images(contents, outdir):
    ...
    for link_tag in contents.select('img'):
        ...
        if link_tag.parent.get('onclick'):
            del link_tag.parent['onclick']
...
def process_article(article_url, template, outdir):
    ...
    contents = dom.select('div#content > div.post')
    if not contents:
...
def scrap(template, outdir, index = 1):
    url = 'https://www.gridsagegames.com/blog/' if index == 1 else rf'https://www.gridsagegames.com/blog/page/{index}' 
    while (url is not None):
        dom = bs4(fetch_url(url), 'html.parser')
        for link in dom.select('h2.entry-title > a[href]'):
